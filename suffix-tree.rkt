...
(define (get-ch-branch st ch)
  ;'your-code-here
  (cond
    ; verific daca st e gol
    ((st-empty? st) #f) 
    ; verific daca 
    ; ch(caracter dat) = 
    ; primul caracter al etichetei primei ramuri a arborelui de sufixe
    ; #t afiseaza first-branch st
    ((equal? ch (car (get-branch-label (first-branch st)))) (first-branch st))
    ; #f prin recursivitate urmarim obtinerea
    ; celorlalte ramuri pt a continua cautarea
