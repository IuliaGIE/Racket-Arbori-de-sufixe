...
(define (get-ch-words words ch)
  ; concatenam rezultatele din map intr-o singura lista
  (foldr append '()
    ; pentru fiecare cuvant din lista de cuvinte
    (map (lambda (word)
      ; verificam cuvantul sa nu fie null
      (if (and (not (null? word))
      ; primul caracter al cuvantului = ch
      (char=? (car word) ch))
        ; returneaza lista cu acel cuvant
        (list word)
        ; #f lista nula
        '()))
    ; aplicam lambda fiecarui cuvant din lista
    words)))
...
(define (cst-func suffixes)
  ; cel mai lung prefix al sufixelor
  (cons (longest-common-prefix-of-list suffixes)
    ;  fct anonima pe fiecare sufix pentru a elimina prefixul comun
    (map (lambda (suffix)
      ; elimina prfixul comun din fiecare sufix
      (remove-prefix suffix (longest-common-prefix-of-list suffixes)))
      suffixes)))
...
(define (suffixes->st labeling-func suffixes alphabet)
  ...
  (if (null? alphabet)
    ...
    (map subtree (filter (lambda (ch) (not (null? (subtree ch)))) alphabet))))
  ...
  (lambda (labeling-func)
    (lambda (text)
      (suffixes->st 
        ...
        (sort (cons #\$ (remove-duplicates text)) char<?)))))
